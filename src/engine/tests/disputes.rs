...
fn disputing_a_deposit_moves_its_funds_from_available_to_held() {
    ...
    apply_successfully(&mut engine, deposit(7, 600, "3.2500"));
...
fn resolving_a_dispute_returns_held_funds_to_available() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(7, 610, "3.2500"));
    apply_successfully(&mut engine, dispute(7, 610));

    let outcome = apply(&mut engine, resolve(7, 610));

    assert_eq!(outcome, ApplyOutcome::Applied);
    let resolved_account = account(&engine, 7);
    assert_eq!(resolved_account.available, decimal("3.2500"));
    assert_eq!(resolved_account.held, Decimal::ZERO);
}

#[test]
fn a_resolved_deposit_can_be_disputed_again() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(7, 620, "3.2500"));
    apply_successfully(&mut engine, dispute(7, 620));
    apply_successfully(&mut engine, resolve(7, 620));
...
fn charging_back_a_dispute_removes_held_funds_and_locks_the_account() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(9, 630, "5.0000"));
    apply_successfully(&mut engine, dispute(9, 630));
...
fn a_chargeback_locks_the_account_with_another_dispute_still_held() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(9, 631, "5.0000"));
    apply_successfully(&mut engine, deposit(9, 632, "2.5000"));
    apply_successfully(&mut engine, dispute(9, 631));
    apply_successfully(&mut engine, dispute(9, 632));
...
fn a_dispute_can_make_available_funds_negative_after_money_was_spent() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(2, 640, "5.0000"));
    apply_successfully(&mut engine, withdrawal(2, 641, "4.2500"));

    apply_successfully(&mut engine, dispute(2, 640));

    let disputed_account = account(&engine, 2);
    assert_eq!(disputed_account.available, decimal("-4.2500"));
    assert_eq!(disputed_account.held, decimal("5.0000"));
    assert_eq!(disputed_account.total, decimal("0.7500"));
}
...
fn controls_for_another_client_are_ignored_without_creating_an_account() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(3, 652, "2.0000"));

    let mismatched_dispute = apply(&mut engine, dispute(4, 652));
    apply_successfully(&mut engine, dispute(3, 652));
    ...
    let owner_account = account(&engine, 3);
    assert_eq!(owner_account.available, Decimal::ZERO);
    assert_eq!(owner_account.held, decimal("2.0000"));
    assert_eq!(
...
fn a_successful_withdrawal_is_not_disputable() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(3, 655, "5.0000"));
    apply_successfully(&mut engine, withdrawal(3, 656, "2.0000"));
    ...
    let unchanged_account = account(&engine, 3);
    assert_eq!(unchanged_account.available, decimal("3.0000"));
    assert_eq!(unchanged_account.held, Decimal::ZERO);
}

#[test]
fn controls_that_do_not_match_the_current_dispute_state_are_ignored() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(5, 660, "2.0000"));

    let resolve = apply(&mut engine, resolve(5, 660));
    let chargeback = apply(&mut engine, chargeback(5, 660));
    apply_successfully(&mut engine, dispute(5, 660));
...
fn every_later_transaction_for_a_charged_back_account_is_ignored() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(10, 670, "4.0000"));
    apply_successfully(&mut engine, dispute(10, 670));
    apply_successfully(&mut engine, chargeback(10, 670));
...
fn a_locked_account_does_not_stop_another_client_from_processing() {
    let mut engine = Engine::new();
    apply_successfully(&mut engine, deposit(10, 680, "4.0000"));
    apply_successfully(&mut engine, dispute(10, 680));
    apply_successfully(&mut engine, chargeback(10, 680));
